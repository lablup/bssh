...
use crate::ssh::ssh_config::env_cache::{EnvironmentCache, EnvCacheConfig, GLOBAL_ENV_CACHE};
...
mod tests {
    ...
    fn test_path_expansion_uses_cache() {
        ...
        assert!(stats.hits > 0 || stats.misses > 0, "Cache should have been accessed");
        ...
        println!(
            "First expansion took: {:?}, Second expansion took: {:?}", 
            first_duration, 
            second_duration
        );
    ...
    fn test_path_expansion_with_multiple_variables() {
        ...
        let stats = GLOBAL_ENV_CACHE.stats();
        println!("Multi-variable expansion cache stats: hits={}, misses={}", stats.hits, stats.misses);
    }
    ...
    fn test_path_expansion_security_with_unsafe_variables() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("security violation") || e.to_string().contains("Security violation") {
                    println!("Path expansion correctly rejected unsafe variable (expected behavior)");
                    
                    // Check that cache wasn't accessed for unsafe variables
                    let stats = GLOBAL_ENV_CACHE.stats();
                    println!("Cache stats for rejected unsafe variable: hits={}, misses={}", stats.hits, stats.misses);
        ...
        assert!(
            ...
            "Unsafe variable should not be expanded: {}", 
            path_str
        );
        
        let stats = GLOBAL_ENV_CACHE.stats();
        println!("Unsafe variable expansion cache stats: hits={}, misses={}", stats.hits, stats.misses);
    }
    ...
    fn test_cache_performance_improvement() {
        ...
        println!("Performance test cache stats: hits={}, misses={}, hit_rate={:.2}%", 
    ...
    fn test_cache_with_nonexistent_variable() {
        ...
        if stats.hits + stats.misses == 0 {
            ...
            assert!(updated_stats.hits + updated_stats.misses > 0, "Cache should have been accessed for safe variables");
        } else {
            assert!(stats.hits + stats.misses > 0, "Cache should have been accessed");
        }
    }
    ...
    fn test_cache_ttl_behavior() {
        ...
        assert_eq!(result1.as_ref().unwrap(), result3.as_ref().unwrap(),
