...
//! Tracks remote alternate-screen transitions across fragmented PTY output.
...
impl ScreenModeTracker {
    ...
    pub(crate) fn observe(&mut self, input: &[u8]) -> Option<ScreenObservation> {
        self.pending.extend_from_slice(input);
        let mut observed = false;
        let mut consumed = 0;

        for (start, end) in csi_ranges(&self.pending) {
            if let Some(is_alternate) = parse_screen_selection(&self.pending[start..end]) {
                if self.current.is_some_and(|current| current != is_alternate) {
                    self.changed = true;
                }
                self.current = Some(is_alternate);
                observed = true;
            }
            consumed = end;
        }
        ...
        observed.then(|| ScreenObservation {
...
fn parse_screen_selection(sequence: &[u8]) -> Option<bool> {
    let (parameters, is_alternate) = if let Some(value) = sequence
        .strip_prefix(b"\x1b[?")
...
mod tests {
    ...
    #[test]
    fn tracks_fragmented_transitions_and_round_trips() {
        let mut tracker = ScreenModeTracker::new(Some(true));

        assert_eq!(tracker.observe(b"text\x1b[?10"), None);
        assert_eq!(
            tracker.observe(b"49l\x1b[?1049h"),
