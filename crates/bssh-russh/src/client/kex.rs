    ...
    pub fn step(
        ...
        match self.state {
            ClientKexState::Created => {
                ...
                debug!("kex: algorithm: {}", names.kex.as_ref());
                debug!("kex: host key algorithm: {}", names.key.as_str());
                debug!(
                    "kex: client->server cipher: {} MAC: {} compression: {}",
                    names.cipher.as_ref(),
                    names.client_mac.as_ref(),
                    names.client_compression.name(),
                );
                debug!(
                    "kex: server->client cipher: {} MAC: {} compression: {}",
                    names.cipher.as_ref(),
