...
async fn test_localhost_upload_download_roundtrip() {
    ...
    let nodes = vec![Node::new("localhost".to_string(), 22, std::env::var("USER").unwrap_or_else(|_| "root".to_string()))];
    let executor = ParallelExecutor::new(nodes, 1, None);
    
    // Test upload
    let remote_path = format!("{}/uploaded_file.txt", remote_temp.path().display());
    let upload_results = executor.upload_file(&local_file, &remote_path).await.unwrap();
    ...
    let download_results = executor.download_file(&remote_path, download_temp.path()).await.unwrap();
...
async fn test_localhost_multiple_file_upload() {
    ...
    for (name, content) in &files {
        ...
        let results = executor.upload_file(&local_file, &remote_path).await.unwrap();
...
async fn test_parallel_execution_with_multiple_nodes() {
    ...
    let executor = ParallelExecutor::new(nodes, 2, None);
